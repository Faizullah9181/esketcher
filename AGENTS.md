...
- **The static demo never fakes Jev.** `VITE_OFFLINE=1` (`npm run build:static`, what `esketcher.faiz-ai.dev` serves) swaps `api` for `createOfflineApi()` in `frontend/src/services/api.ts`. That adapter reads the bundled catalog, saves to localStorage, and rejects every Jev call with code `offline`. Gate new Jev entry points on `OFFLINE`.
